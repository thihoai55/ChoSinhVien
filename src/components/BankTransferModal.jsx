...
export default function BankTransferModal({ post, buyer, transaction, onConfirm, onCancel, onClose }) {
  ...
  // Support either onCancel or onClose prop name (some callers use onClose)
  ...
  const handleConfirm = () => {
    if (validate()) {
      onConfirm?.({
        ...
        shippingFee: String(shippingFeeNum), // Store as numeric string for consistency
        totalAmount: totalAmount,
      });
    }
  };

  // Extract numeric price - handle both string and formatted prices
  const extractPrice = (price) => {
    if (!price) return 0;
    const str = String(price).replace(/[^0-9.-]/g, ''); // Remove all non-numeric chars except . and -
    return parseFloat(str) || 0;
  };
  const productPrice = extractPrice(post?.price);
  const shippingFeeNum = extractPrice(bankInfo.shippingFee);
  const totalAmount = productPrice + shippingFeeNum;
  ...
  return (
    <div style={overlay} onClick={closeHandler}>
      <div style={modal} onClick={(e) => e.stopPropagation()}>
        <div style={header}>
          <h2 style={title}>Thông tin chuyển khoản</h2>
          <button style={closeBtn} onClick={closeHandler}>
            ✕
          </button>
        </div>
        ...
        <div style={section}>
          ...
          <div style={priceInfo}>
            ...
            <div style={priceRow}>
              ...
              <span style={priceValue}>{shippingFeeNum.toLocaleString('vi-VN')} đ</span>
            </div>
            <div style={totalRow}>
