...
export default function PaymentNotificationModal({ 
  ...
  const bankInfo = notification?.bankTransferInfo || {};
  const post = notification?.post || {};
  const shippingFee = parseFloat(bankInfo.shippingFee || 0);
  const productPrice = parseFloat(post?.price || 0);
  const totalAmount = productPrice + shippingFee;

  const handleConfirmPayment = () => {
    setPaymentStatus('completed');
    // Gọi callback sau 2 giây để cho phép người dùng thấy thông báo thành công
    setTimeout(() => {
      onConfirmPayment?.();
    }, 2000);
  };
